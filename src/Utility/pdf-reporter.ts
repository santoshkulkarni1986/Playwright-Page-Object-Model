...
export default class PdfReporterPlaywright implements Reporter {
  ...
  async onEnd(result: FullResult) {
    ...
    if (!fs.existsSync(reportPath)) {
      console.error("HTML report not found. Run tests with HTML reporter enabled.");
      return;
    }
    ...
    await page.evaluate(() => {
      document.querySelectorAll('.test-details').forEach(el => (el as HTMLElement).style.display = 'block');
